...
/** Closes the current scope, if it exists **/
   public void closeScope() throws EmptySTException {
       ...
       if (!check){
           ...
           currentScope.clear();
           currentScope = scopeList.peekLast(); // this will set currentScope to null if it's empty after
       }
    }
    /* Helper for close scope, checks for hashmap*/
    /* So if current scope was set to null when closing the previous one, it returns false and throw an exception*/
...
/* Inserts the given symb into the table and throws empty exception if it's null */
/* Uses same helper function as the close scope check*/
   public void insert(Symb s) throws EmptySTException, DuplicateException {
       ...
       if (!check){
           ...
           if(currentScope.containsValue(s.name())){ // current scope contains the value
               //Throw exception
...
/* Local Lookup checks if the current scope hash map contains the given string */
/* Uses same helper function as the close scope check */
...
/* Global Lookup checks if the current scope hash map contains the given string */
/* If it doesn't, it checks all the other scopes until it finds it or hits null */
